...
############################################################################################################
    ...
    def distance_to_goal(self):
        if self.side == WorldModel.SIDE_L:
            ...
            goal_pos = (-55,0)
        return self.euclidean_distance(goal_pos,self.abs_coords)
    ...
    def enemies_in_range(self):
        ...
        for p in self.players:
            if p.side != self.side:
                ...
                #print self.get_distance_to_point(p_coords)
                if (self.get_distance_to_point(p_coords)) < self.enemy_range:
    ...
    def is_free_path_to_goal(self):
        ...
        if self.side == WorldModel.SIDE_L:
            ...
            goal_pos = (-55,0)
        for p in self.players:
            if p.side != self.side and p != None:
                p_coords = self.get_object_absolute_coords(p)
                if(self.is_between(goal_pos,self.abs_coords,p_coords)):
                    flag = 1
                    break
        #print "Checking for enemy in middle"      
        if flag == 0:
            return 1
        else:
            return 0
    ...
    def get_required_episodes(self):
        return self.training_episodes


    def is_terminal(self):
        if (self.last_message == WorldModel.RefereeMessages.GOAL_L or
            self.last_message == WorldModel.RefereeMessages.GOAL_R or
            self.last_message == WorldModel.RefereeMessages.FOUL_L or
            self.last_message == WorldModel.RefereeMessages.FOUL_R or
    ...
    def should_shoot(self):
        ...
        ## Returns if we should go for shoot or not

        if self.distance_to_goal < 32 and self.is_ball_kickable :
            return 1
        else:
            return 0
    ...
    def get_nearest_team_dist(self):
        
        distance=[]
        for p in self.players:
            if p.side == self.side and p != None:
                p_coords = self.get_object_absolute_coords(p)
                distance.append(self.get_distance_to_point(p_coords))
        try:
    ...
    def get_nearest_enemy_dist(self):
    ...
    def get_nearest_teammate(self):
        ...
        try:
            nearest = min(distances)[1]
            #self.nearest_team_dist = min(distances)[0]
            return nearest
        except:
            return None
    ...
    def get_nearest_enemy(self):
        ...
        try:
            nearest = min(distances)[1]
            self.nearest_enemy_dist = min(distances)[0]
            return nearest
        except:
            return None
...
## Code for approximate Q learning
